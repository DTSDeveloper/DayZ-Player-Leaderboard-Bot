...
process.on("uncaughtException", err => {
  if (err.code === "ECONNRESET") {
    console.warn("⚠️ ECONNRESET ignorado (FTP bugado)");
    return;
  }
...
process.on("unhandledRejection", err => {
  if (err?.code === "ECONNRESET") {
    console.warn("⚠️ ECONNRESET ignorado (promise)");
    return;
  }
...
async function loadFiles() {
  ...
  try {
    if (!fs.existsSync("./playerJsons")) {
      fs.mkdirSync("./playerJsons");
    }

    let remoteFiles = [];

    // ===== TENTA FTP (SE FALHAR, IGNORA) =====
    try {
      const ftp = new basicFTP.Client(20000);
      ftp.ftp.useEPSV = false;
      ftp.prepareTransfer = enterPassiveModeIPv4;
      ...
      for (const dir of FTP_PATH) {
        await ftp.cd(dir);
      }

      remoteFiles = (await ftp.list())
        .filter(f => f.name.endsWith(".json"))
        .map(f => f.name);

      ftp.close();
    } catch (e) {
      console.warn("⚠️ FTP indisponível, usando cache local");
    }

    // ===== SE TEM LISTA, TENTA BAIXAR =====
    for (const file of remoteFiles) {
      try {
        ...
        await ftp.downloadTo("./playerJsons/" + file, file);
        ftp.close();

        await new Promise(r => setTimeout(r, 300));
      } catch {
        // IGNORA falha de arquivo individual
      }
    }

    // ===== SEMPRE CARREGA DO DISCO =====
    players = [];

    const files = fs.readdirSync("./playerJsons");
    ...
    console.log("✅ Players carregados:", players.length);
...
function loadPlayer(file) {
  ...
  if (!p.name || Number.isNaN(p.timeSurvived) || p.timeSurvived < 0) {
    console.warn("⚠️ Player inválido:", file);
    return;
  }
